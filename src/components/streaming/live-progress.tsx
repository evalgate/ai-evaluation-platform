...
export function LiveProgress({
  ...
  useEffect(() => {
    ...
    const eventSource = new EventSource(`/api/stream?clientId=${clientId}&channels=evaluation`);
    ...
    eventSource.onmessage = (event) => {
      ...
      try {
        ...
        if (event.type === "evaluation_progress" && data.evaluationId === evaluationId) {
          setProgress((prev) => ({
            ...prev,
            ...data,
        ...
        } else if (event.type === "evaluation_started" && data.evaluationId === evaluationId) {
          setProgress({
        ...
        } else if (event.type === "evaluation_completed" && data.evaluationId === evaluationId) {
          setProgress((prev) => ({
            ...prev,
            status: "completed",
            progress: 100,
        ...
        } else if (event.type === "test_case_started" && data.evaluationId === evaluationId) {
          setProgress((prev) => {
            if (!prev) return prev;

            const updatedTestCases = prev.testCases.map((tc) =>
              tc.id === data.testCaseId
                ? { ...tc, status: "running" as const, startedAt: new Date().toISOString() }
                : tc,
            );
  ...
  if (!isVisible) {
    return (
      <Button variant="outline" size="sm" onClick={() => setIsVisible(true)} className={className}>
      ...
      <CardHeader className={cn("pb-3", compact && "pb-2")}>
        <div className="flex items-center justify-between">
          <div className="flex items-center gap-2">
            ...
            <div>
              <CardTitle className={cn("text-lg", compact && "text-base")}>Live Progress</CardTitle>
              {!compact && (
                <CardDescription>Real-time evaluation progress tracking</CardDescription>
              )}
            </div>
          </div>
      ...
      <CardContent className={cn("space-y-4", compact && "space-y-2")}>
        ...
        <div className="space-y-2">
          ...
          <div className="flex items-center justify-between text-xs text-gray-500">
            <span>
              {progress?.completedTests || 0} / {progress?.totalTests || 0} tests
            </span>
            <span>{formatDuration(progress?.startedAt, progress?.completedAt)}</span>
          </div>
        </div>
        ...
        <div className="flex items-center gap-2">
          <Badge
            variant={progress?.passedTests === progress?.completedTests ? "default" : "secondary"}
          >
          ...
          <>
            ...
            <div className="space-y-2">
              <div className="flex items-center gap-2 text-sm">
                ...
                <span className="font-medium">Current: {progress.currentTest}</span>
              </div>
                ...
                {progress.testCases.slice(-5).map((testCase) => (
                  ...
                  >
                    <div className="flex items-center gap-2">
                      ...
                      <span className="truncate max-w-[200px]">{testCase.name}</span>
                    </div>
                    <div className="flex items-center gap-2">
                      ...
                      <span className="text-gray-500">
                        {formatDuration(testCase.startedAt, testCase.completedAt)}
                      </span>
                    </div>
                  </div>
                ))}
