...
async function completeWorkflow() {
	...
	for (let i = 0; i < responses.length; i++) {
		...
		if (!result.passed) {
			console.log(
				`  Failures: ${result.failures.map((f) => f.message).join(", ")}`,
			);
		}
	}
	...
	const allTraces = [];
	for await (const trace of autoPaginate(async (offset, limit) => {
	...
	const passedTests = testSuite
		.getCases()
		.filter((_, i) => responses[i].output.length > 20).length;
