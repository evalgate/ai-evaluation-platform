...
vi.mock("./cache", () => {
  ...
  return {
    ...
    RequestCache: class RequestCache {
      ...
      set(method: string, url: string, data: unknown, _ttl: number, params?: unknown) {
        const key = makeKey(method, url, params);
...
describe("AIEvalClient.request", () => {
  ...
  it("caches GET responses and reuses data without re-fetching", async () => {
    const client = new AIEvalClient({ apiKey: "test", baseUrl: "http://localhost", timeout: 1000 });
  ...
  it("propagates non-ok responses as SDK errors", async () => {
    const client = new AIEvalClient({ apiKey: "test", baseUrl: "http://localhost" });
    const fetchMock = vi.fn().mockResolvedValue({
      ok: false,
      status: 429,
    ...
    await expect(client.request("/api/fail", { method: "GET" })).rejects.toHaveProperty(
      "code",
      "RATE_LIMIT_EXCEEDED",
    );
  ...
  it("throws a TIMEOUT SDK error when fetch aborts", async () => {
    ...
    await expect(client.request("/api/timeout", { method: "GET" })).rejects.toMatchObject({
      code: "TIMEOUT",
    });
  });
