...
import { migrateConfig, validateConfigFile, previewMigration } from "../../cli/migrate";
...
describe("COMPAT-203: Config → DSL Migration Generator", () => {
  ...
  describe("Migration generation", () => {
    it("should generate DSL from simple config", async () => {
      ...
      expect(outputContent).toContain("// Auto-generated EvalAI DSL from configuration");
      expect(outputContent).toContain(
        "import { defineEval, createResult } from '@pauly4010/evalai-sdk';",
      );
      expect(outputContent).toContain('defineEval("simple-test", async (context) => {');
      expect(outputContent).toContain(
    ...
    it("should generate DSL from config with multiple suites", async () => {
      ...
      expect(outputContent).toContain('defineEval("test-1-1", async (context) => {');
      expect(outputContent).toContain('defineEval("test-2-1", async (context) => {');
    ...
    it("should handle complex test configurations", async () => {
      ...
      expect(outputContent).toContain('defineEval("complex-test", async (context) => {');
    ...
    it("should create output directory if it doesn't exist", async () => {
      ...
      const nestedOutputPath = path.join(tempDir, "nested", "dir", "migrated.spec.ts");
  ...
  describe("Integration with TestSuite", () => {
    it("should work with actual TestSuite instances", async () => {
      ...
      expect(outputContent).toContain('defineEval("integration-test", async (context) => {');
