...
describe("COMPAT-204: Dual-path Execution Toggle", () => {
  ...
  beforeEach(async () => {
    ...
    tempDir = await fs.mkdtemp(path.join(os.tmpdir(), "evalai-execution-test-"));
  ...
  describe("Validation", () => {
    ...
    it("should warn about mixed projects", async () => {
      ...
      expect(validation.warnings[0]).toContain("both spec files and legacy config");
    });
    ...
    it("should error on incompatible mode requests", async () => {
      ...
      expect(validation.errors[0]).toContain("Spec mode requested but no spec files found");
    });
  });
