...
import { createEvalRuntime, disposeActiveRuntime } from "../../runtime/registry";
...
describe("COMPAT-205: Backward Compatibility Test Suite", () => {
  ...
  describe("Legacy project compatibility", () => {
    ...
    it("should run legacy tests without modifications", async () => {
      ...
      const config = JSON.parse(
        await fs.readFile(path.join(legacyProjectDir, "evalai.config.json"), "utf-8"),
      );
      const suite = createTestSuite("legacy-tests", config);
    ...
    it("should adapt to DSL without breaking behavior", async () => {
      ...
      try {
        ...
        // Execute first spec
        const result = await executor.executeSpec(registeredSpecs[0], "Hello world");
  ...
  describe("Spec-based project compatibility", () => {
    ...
    it("should detect spec execution mode", async () => {
      ...
      expect(config.specFiles.some((f) => f.includes("greeting.spec.ts"))).toBe(true);
      expect(config.specFiles.some((f) => f.includes("complex.spec.ts"))).toBe(true);
    });
    ...
    it("should generate deterministic reports", async () => {
      ...
      try {
        ...
        report.addResult(spec.id, spec.name, spec.filePath, spec.position, "test input", result);
  ...
  describe("Mixed project compatibility", () => {
    ...
    it("should detect mixed project and prefer spec mode", async () => {
      ...
      const recommended = require("../../runtime/execution-mode").getRecommendedExecutionMode(
        config,
      );
    ...
    it("should run both legacy and spec tests", async () => {
      ...
      const legacyConfig = JSON.parse(
        await fs.readFile(path.join(mixedProjectDir, "evalai.config.json"), "utf-8"),
      );
  ...
  describe("Transparent tool usage", () => {
    ...
    it("should work with explain command", async () => {
      ...
      try {
        ...
        report.addResult(spec.id, spec.name, spec.filePath, spec.position, "success", result);
  ...
  describe("Regression prevention", () => {
    it("should prevent regressions in legacy test execution", async () => {
      ...
      const { createTestSuite } = require("@pauly4010/evalai-sdk");
      const config = JSON.parse(
        await fs.readFile(path.join(tempDir, "regression-legacy", "evalai.config.json"), "utf-8"),
      );
    ...
    it("should prevent regressions in spec test execution", async () => {
      ...
      try {
        ...
        runtime.runtime.defineEval("regression-spec-2", async (context: any) => {
        ...
        const result1 = await executor.executeSpec(specs[0], "REGRESSION INPUT");
    ...
    it("should maintain score bucket consistency", async () => {
      ...
      try {
        ...
        const mediumScores = results.filter((r) => r.score >= 70 && r.score < 90);
