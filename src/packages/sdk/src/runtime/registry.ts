...
import { SpecRegistrationError, RuntimeError } from "./types";
...
class EvalRuntimeImpl implements EvalRuntime {
  ...
  register(spec: EvalSpec): void {
    ...
    if (this.specs.has(spec.id)) {
      throw new SpecRegistrationError(`Specification with ID '${spec.id}' already exists`, {
        specId: spec.id,
        specName: spec.name,
  ...
  snapshot(): RuntimeSnapshot {
    ...
    const serializedSpecs: SerializedSpec[] = Array.from(this.specs.values()).map((spec) => ({
      id: spec.id,
      name: spec.name,
      filePath: spec.filePath,
  ...
  load(snapshot: RuntimeSnapshot): void {
    ...
    if (snapshot.runtimeId !== this.id) {
      throw new RuntimeError("Snapshot runtime ID does not match current runtime");
    }

    if (snapshot.namespace !== this.namespace) {
      throw new RuntimeError("Snapshot namespace does not match current runtime");
    }
  ...
  private validateSpec(spec: EvalSpec): void {
    ...
    if (!spec.executor || typeof spec.executor !== "function") {
      throw new SpecRegistrationError("Specification executor is required and must be a function", {
        spec,
      });
    }
    ...
    if (
      ...
      throw new SpecRegistrationError("Specification AST position is required", {
        spec,
      });
    }
    ...
    if (!spec.id || spec.id.length !== 20) {
      throw new SpecRegistrationError("Specification ID must be 20 characters long", {
        spec,
      });
    }
  }
 ...
 */
export function createEvalRuntime(projectRoot: string = process.cwd()): RuntimeHandle {
