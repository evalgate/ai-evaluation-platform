...
import { fetchQualityLatest, type QualityLatestData } from "./api";
import {
	type EvalAIConfig,
	findConfigPath,
	loadConfig,
...
export function checkJudgeConfig(
	cwd: string,
	config: EvalAIConfig | null,
): CheckResult & { judgeInfo: JudgeDiagnosticInfo } {
	...
	if (!judge) {
		return {
			...
			status: "skip",
			message:
				"No judge config found (optional until judge credibility is enabled)",
			judgeInfo: { configured: false },
		};
	}

	if (
		typeof judge.bootstrapSeed === "number" &&
		...
		return {
			...
			status: "fail",
			message:
				"judge.bootstrapSeed must be a finite integer for deterministic CI output",
			remediation:
				"Set judge.bootstrapSeed to an integer (recommended default: 42)",
			judgeInfo: {
				configured: true,
	...
	const judgeInfo: JudgeDiagnosticInfo = {
		configured: true,
		bootstrapIterations: judge.bootstrapIterations,
		bootstrapSeed: judge.bootstrapSeed,
	};
	...
	if (!judge.labeledDatasetPath) {
		return {
			...
			remediation:
				"Add judge.labeledDatasetPath in evalgate.config.json to enable judge alignment checks",
			judgeInfo,
		};
	}
	...
	if (!judgeInfo.labeledDatasetExists) {
		return {
			...
			remediation:
				"Run 'evalgate label' to create a labeled golden dataset, or update judge.labeledDatasetPath",
			judgeInfo,
		};
	}
...
export function checkGoldenSetHealth(
	cwd: string,
	config: EvalAIConfig | null,
): CheckResult {
	...
	try {
			...
			try {
				...
				if (caseIds.has(labeled.caseId)) {
					return {
						...
						remediation:
							"Remove duplicate entries or regenerate labeled dataset",
					};
				}
				...
				if (labeled.label === "pass") {
					...
					if (labeled.failureMode) {
						failureModes.set(
							labeled.failureMode,
							(failureModes.get(labeled.failureMode) || 0) + 1,
						);
					}
				}
		...
		if (total < 30) {
			issues.push(
				`Small dataset size (${total} samples, recommend ≥30 for statistical significance)`,
			);
		}

		if (passRate > 0.95) {
			issues.push(
				`Very high pass rate (${(passRate * 100).toFixed(1)}%), may lack challenging cases`,
			);
		} else if (passRate < 0.3) {
			issues.push(
				`Very low pass rate (${(passRate * 100).toFixed(1)}%), may be too difficult or labels incorrect`,
			);
		}
		...
		if (staleCount > total * 0.5) {
			issues.push(
				`${staleCount} samples (${((staleCount / total) * 100).toFixed(1)}%) are older than 6 months`,
			);
		}
		...
		if (issues.length > 0) {
			return {
				...
				remediation:
					"Consider adding more diverse cases, reviewing labels, or updating stale samples",
			};
		}
...
function roundPct(value: number, precision: number = 1): number {
...
export async function checkEvalAccess(
	...
	evaluationId: string,
	baseline: string,
): Promise<CheckResult & { quality?: QualityLatestData }> {
...
export function checkJudgeCredibilityWarnings(
		...
		return [
			{
				...
				status: "skip",
				message:
					"No judge alignment metrics on latest run (cannot assess correction viability)",
			},
		];
	}
	...
	if (typeof tpr === "number" && typeof tnr === "number") {
		...
		if (discriminativePower <= MIN_DISCRIMINATIVE_POWER) {
			checks.push({
				id: "judge_correction_viability",
				label: "Judge correction viability",
				status: "warn",
				message: `Judge appears near-random (TPR + TNR - 1 = ${discriminativePower.toFixed(3)} <= ${MIN_DISCRIMINATIVE_POWER}). Correction will be skipped and raw rate used.`,
				remediation:
					"Improve judge prompt/calibration and relabel sample data before enforcing corrected-rate gates",
			});
		} else {
			checks.push({
				id: "judge_correction_viability",
				label: "Judge correction viability",
				status: "pass",
				message: `Judge discriminative power is ${discriminativePower.toFixed(3)} (> ${MIN_DISCRIMINATIVE_POWER})`,
			});
		}
	} else {
		checks.push({
			...
			status: "warn",
			message:
				"Missing TPR/TNR metrics; cannot determine whether correction is reliable",
			remediation:
				"Ensure judge alignment metrics (TPR/TNR) are computed and published for the target evaluation",
		});
	}

	if (typeof sampleSize === "number") {
		if (sampleSize < MIN_BOOTSTRAP_SAMPLE_SIZE) {
			checks.push({
				id: "judge_ci_sample_size",
				label: "Judge CI sample size",
				status: "warn",
				message: `Sample size ${sampleSize} < ${MIN_BOOTSTRAP_SAMPLE_SIZE}; bootstrap CI will be skipped`,
				remediation:
					"Collect at least 30 labeled samples before relying on CI bounds",
			});
		} else {
			checks.push({
				id: "judge_ci_sample_size",
		...
		checks.push({
			...
			remediation:
				"Publish judge sample size metrics so CI sufficiency checks can run",
		});
	}
...
export function checkReplayDecisionReadiness(
	cwd: string,
	config?: EvalAIConfig | null,
): CheckResult {
	...
	try {
		if (!fs.existsSync(runsDir)) {
			return {
				...
				status: "warn",
				message:
					"Budget configured but no previous runs found in .evalgate/runs/",
				remediation:
					"Run at least one evaluation before using replay-decision command",
			};
		}
		...
		if (runFiles.length === 0) {
			return {
				...
				status: "warn",
				message:
					"Budget configured but no run artifacts found in .evalgate/runs/",
				remediation:
					"Run at least one evaluation before using replay-decision command",
			};
		}
