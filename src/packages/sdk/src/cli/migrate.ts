...
function generateSummary(
	...
	const totalTests = suites.reduce(
		(sum, { suite }) => sum + suite.getTests().length,
		0,
	);
...
export async function migrateConfig(options: MigrateOptions): Promise<void> {
	try {
		...
		if (suites.length === 0) {
			throw new Error(
				"No test suites found in config file. Check config structure.",
			);
		}
		...
		const content = [
			...
			...suites.map(({ name, suite }) =>
				generateSuiteDSL(name, suite, options),
			),
...
export async function validateConfigFile(filePath: string): Promise<boolean> {
	try {
		...
		if (!hasTests) {
			throw new Error(
				"Config file must contain 'tests', 'suites', or 'testSuites' property",
			);
		}
