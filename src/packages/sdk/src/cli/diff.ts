...
export function validateSchemaVersion(report: RunResult): void {
	if (!report.schemaVersion) {
		throw new Error(
			`RunReport missing schemaVersion. This report was generated by an older version of EvalAI.\n` +
...
async function findBaselineRun(): Promise<RunResult | null> {
	...
	throw new Error(
		"No baseline run found. Set a baseline with 'evalai diff --base <runId> --head last --set-baseline' or create .evalai/baseline-run.json.",
	);
}
...
async function findPreviousRun(): Promise<RunResult | null> {
	...
	try {
		...
		if (index.length < 2) {
			throw new Error(
				"Need at least 2 runs to use 'last' shortcut. Run 'evalai run --write-results' at least twice.",
			);
		}
		...
		throw new Error(
			"No run history found. Run 'evalai run --write-results' first.",
		);
	}
}
 ...
 */
async function findLastRunForBranch(branch: string): Promise<RunResult | null> {
...
export async function runDiffCLI(options: DiffOptions): Promise<void> {
	try {
		...
		console.error(
			`EvalAI ERROR: ${error instanceof Error ? error.message : String(error)}`,
		);
