...
import { generateManifest, writeManifest, readManifest, readLock } from "./manifest";
 ...
 */
export async function discoverSpecs(options: { manifest?: boolean } = {}): Promise<DiscoveryStats> {
  try {
    ...
    if (options.manifest) {
      ...
      const manifest = await generateManifest(specs, projectRoot, project.name, executionMode);
 ...
 * Get project metadata
 */
async function getProjectMetadata(projectRoot: string): Promise<DiscoveryStats["project"]> {
 ...
 */
async function analyzeSpecifications(specFiles: string[]): Promise<SpecAnalysis[]> {
...
function analyzeSpecFile(filePath: string, content: string): SpecAnalysis {
  ...
  const usesTools =
    content.includes("tool:") || content.includes("function.") || content.includes("call(");

  // Check for assertions
  const hasAssertions =
    content.includes("assert") || content.includes("expect") || content.includes("should");
...
function extractTags(content: string): string[] {
  ...
  if (descriptionMatch) {
    ...
    if (description.includes("safety") || description.includes("security")) tags.push("safety");
    if (description.includes("accuracy") || description.includes("precision"))
      tags.push("accuracy");
    if (description.includes("agent") || description.includes("autonomous")) tags.push("agents");
    if (description.includes("tool") || description.includes("function")) tags.push("tools");
    if (description.includes("latency") || description.includes("speed")) tags.push("performance");
    if (description.includes("hallucination") || description.includes("fact")) tags.push("factual");
    if (description.includes("bias") || description.includes("fairness")) tags.push("bias");
    if (description.includes("privacy") || description.includes("pii")) tags.push("privacy");
  }
...
function analyzeComplexity(content: string): "simple" | "medium" | "complex" {
  ...
  const hasExternalCalls =
    content.includes("fetch") || content.includes("http") || content.includes("api");
