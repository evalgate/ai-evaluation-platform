...
export async function runPromote(args: string[]): Promise<number> {
	...
	if (parsed.list) {
		const data = await fetchAPI(
			"/api/candidates?status=quarantined&auto_promote_eligible=true",
			fetchOpts,
		);
	...
	if (parsed.auto) {
		...
		for (const c of candidates) {
			try {
				...
				await fetchAPI(`/api/candidates/${c.id}/promote`, {
					...fetchOpts,
					method: "POST",
					body,
				});
				...
				console.error(
					`  ✖ Failed to promote [${c.id}]: ${err instanceof Error ? err.message : String(err)}`,
				);
			}
		}
	...
	const result = await fetchAPI(
		`/api/candidates/${parsed.candidateId}/promote`,
		{
			...fetchOpts,
			method: "POST",
			body,
	...
	if (result.test_case_id) {
		console.log(
			`    → Test case #${result.test_case_id} in evaluation #${result.evaluation_id}`,
		);
	}
	return 0;
}
