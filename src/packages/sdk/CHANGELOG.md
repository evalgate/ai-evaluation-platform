...
- **Platform compatibility alignment** — Bumped `SDK_VERSION` and `SPEC_VERSION` to `3.0.3` to align request headers and release metadata with the platform hardening release.

### Fixed

- **Release integrity** — TypeScript SDK release metadata now tracks the platform reliability hardening release, including scoped workflow authorization fixes, evaluation execution timeout/failure hardening, standalone build regression coverage, and deterministic usage-accounting idempotency keys.
