...
if (subcommand === "init") {
    ...
    const code = (0, analyze_1.runAnalyze)(argv.slice(1));
    process.exit(code);
}
else if (subcommand === "label") {
    (0, label_1.runLabel)(argv.slice(1))
        .then((code) => process.exit(code))
        .catch((err) => {
        console.error(`EvalGate ERROR: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
    ...
    (0, cluster_1.runCluster)(argv.slice(1))
        .then((code) => process.exit(code))
        .catch((err) => {
        console.error(`EvalGate ERROR: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
    ...
    const code = (0, synthesize_1.runSynthesize)(argv.slice(1));
    process.exit(code);
}
else if (subcommand === "auto") {
    (0, auto_1.runAuto)(argv.slice(1))
        .then((code) => process.exit(code))
        .catch((err) => {
        console.error(`EvalGate ERROR: ${err instanceof Error ? err.message : String(err)}`);
        process.exit(1);
