...
class EvalWorker {
	...
	async processRun(
		...
		try {
			...
			computeAndStoreQualityScore(runId, evaluationId, organizationId).catch(
				(error) => {
			...
			const errorMessage =
				error instanceof Error ? error.message : String(error);
			logger.error("Worker: Evaluation run failed", {
				runId,
				error: errorMessage,
			});
	...
	private async triggerMetaJudge(
		...
		logger.info("Worker: Triggering Meta-Judge evaluation", {
			runId,
			evaluationId,
		});
		...
		try {
			...
			const metaJudgeError =
				error instanceof Error ? error.message : String(error);
			logger.error("Worker: Meta-Judge evaluation failed", {
				runId,
	...
	private async evaluateTestCase(
		...
		try {
			...
			if (!providerKey) {
				...
				logger.warn("No provider key found, using heuristic scoring", {
					organizationId,
					provider,
				});
	...
	private heuristicScore(
		...
		const lenRatio =
			...
			Math.max(actual.length, expected.length, 1);
