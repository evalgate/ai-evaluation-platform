...
export function computeQualityScore(i: ScoreInputs): QualityScoreResult {
  ...
  const latencyScore =
    i.avgLatencyMs == null ? 1 :
    i.avgLatencyMs <= 1500 ? 1 :
    i.avgLatencyMs >= 8000 ? 0 :
    1 - ((i.avgLatencyMs - 1500) / (8000 - 1500));

  const costScore =
    i.avgCostUsd == null || i.budgetUsd == null ? 1 :
    i.avgCostUsd <= i.budgetUsd ? 1 :
  ...
  const score01 =
    0.50 * breakdown.passRate +
    0.25 * breakdown.safety +
    0.15 * (0.6 * breakdown.judge + 0.4 * breakdown.schema) +
    0.10 * (0.6 * breakdown.latency + 0.4 * breakdown.cost);

  const flags: string[] = [];
  if (breakdown.safety < 0.95) flags.push('SAFETY_RISK');
  if (breakdown.passRate < 0.90) flags.push('LOW_PASS_RATE');
  if (breakdown.latency < 0.50) flags.push('LATENCY_RISK');
  if (breakdown.cost < 0.50) flags.push('COST_RISK');
