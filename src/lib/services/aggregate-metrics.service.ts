...
export async function computeRunAggregates(
	...
	const judgeRows = await db
		...
		.innerJoin(
			evaluationRuns,
			eq(llmJudgeResults.evaluationRunId, evaluationRuns.id),
		)
		.where(
			and(
	...
	if (total > 0 && provRow) {
		...
		const effectiveWithProvenance =
			costRecordCount > 0 ? withProv + nullProv : withProv;
...
export async function computeAndStoreQualityScore(
	...
	await db.insert(qualityScores).values({
		...
		total: metrics.total,
		traceCoverageRate:
			metrics.traceCoverageRate != null
				? String(metrics.traceCoverageRate)
				: null,
		provenanceCoverageRate:
