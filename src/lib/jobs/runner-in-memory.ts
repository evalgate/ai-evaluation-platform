...
export function createJobTestHarness() {
	return {
		state: harness.state,
		registerHandler: <P = unknown>(
			type: string,
			reg: JobHandlerRegistration<P>,
		) => {
...
export function registerJobHandler<P = unknown>(
	type: string,
	reg: JobHandlerRegistration<P>,
) {
	// Convert the handler to match the harness's expected type
...
export async function runDueJobs(
	...
	const jobLockTtlMs = clampInt(
		opts?.jobLockTtlMs ?? DEFAULT_JOB_LOCK_TTL_MS,
		100,
		300_000,
	);
	...
	try {
		...
		const dueJobs = harness.state.jobs
			.filter(
				(j) =>
					j.status === "pending" &&
					toDate(j.nextRunAt).getTime() <= now.getTime(),
			)
			.sort(
				(a, b) =>
					toDate(a.nextRunAt).getTime() - toDate(b.nextRunAt).getTime() ||
					a.id - b.id,
			)
			.slice(0, limit);
		...
		for (const job of dueJobs) {
			...
			try {
				...
				const simpleHandler = reg.handler as (
					payload: unknown,
				) => Promise<void>;
