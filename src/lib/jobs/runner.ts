...
import { handleWebhookDelivery, WebhookDeliveryError } from "./handlers/webhook-delivery";
...
const RUNNER_TIME_BUDGET_MS = Number(process.env.RUNNER_TIME_BUDGET_MS ?? 20_000);
...
export async function runDueJobs(runId?: string): Promise<RunnerResult> {
  ...
  if (!lockAcquired) {
    logger.info("Job runner skipped — lock held by another invocation", { runId: id });
  ...
  try {
    ...
    for (const candidate of candidates) {
        ...
        .where(
          and(
            ...
            or(
              ...
              and(eq(jobs.status, "running"), lte(jobs.lockedUntil, new Date(claimTime))),
            ),
          ),
        )
      ...
      // ── Payload validation ──────────────────────────────────────────────────
      const validation = validatePayload(candidate.type as JobType, candidate.payload);
...
async function acquireRunnerLock(runId: string, nowMs: number): Promise<boolean> {
  ...
  try {
    ...
    const res = await db
      ...
      .where(and(eq(jobRunnerLocks.lockName, "default"), lt(jobRunnerLocks.lockedUntil, nowMs)))
...
async function releaseRunnerLock(runId: string, nowMs: number): Promise<void> {
  try {
    await db
      ...
      .where(and(eq(jobRunnerLocks.lockName, "default"), eq(jobRunnerLocks.lockedBy, runId)));
  } catch {
...
async function reclaimExpiredJobs(nowMs: number): Promise<number> {
  try {
    const reclaimed = await db
      ...
      .where(and(eq(jobs.status, "running"), lt(jobs.lockedUntil, new Date(nowMs))))
      .returning({ id: jobs.id });
