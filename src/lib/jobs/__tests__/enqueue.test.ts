...
describe("enqueue()", () => {
	...
	it("returns existing id when idempotency key already exists (no duplicate insert)", async () => {
		...
		const id1 = await enqueue("webhook_delivery", validWebhookPayload, {
			idempotencyKey: key,
		});
		const id2 = await enqueue("webhook_delivery", validWebhookPayload, {
			idempotencyKey: key,
		});
		...
		expect(
			harness.state.jobs.filter((j: unknown) => j.idempotencyKey === key),
		).toHaveLength(1);
	});
	...
	it("inserts multiple jobs when no idempotency key is provided", async () => {
		...
		const id1 = await enqueue("webhook_delivery", {
			...validWebhookPayload,
			webhookId: 1,
		});
		const id2 = await enqueue("webhook_delivery", {
			...validWebhookPayload,
			webhookId: 2,
		});
	...
	it("respects custom runAt option", async () => {
		...
		expect(harness.state.jobs[0].nextRunAt.getTime()).toBeCloseTo(
			future.getTime(),
			-2,
		);
	});
