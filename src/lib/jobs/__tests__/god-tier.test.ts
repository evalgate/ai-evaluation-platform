...
describe("enqueue() — god-tier", () => {
	...
	describe("Phase 1A: atomic idempotency", () => {
		...
		it("returns same ID on duplicate idempotencyKey (conflict path)", async () => {
			...
			expect(
				harness.state.jobs.filter(
					(j: unknown) => j.idempotencyKey === "dup-key",
				),
			).toHaveLength(1);
		});
	...
	describe("Phase 1B: payload validation at enqueue", () => {
		...
		it("rejects deeply nested payloads", async () => {
			...
			await expect(
				enqueue("webhook_delivery", nested, { skipValidation: true }),
			).rejects.toThrow("Payload too deep");
		});
		...
		it("rejects invalid Zod payload when validation is not skipped", async () => {
			...
			await expect(enqueue("webhook_delivery", { bad: true })).rejects.toThrow(
				"Invalid payload",
			);
		});
...
describe("JobErrorCodes — taxonomy", () => {
	it("contains all required error codes", async () => {
		...
		expect(JobErrorCodes.LOCK_TIMEOUT_RECLAIMED).toBe(
...
describe("WebhookDeliveryError", () => {
	it("carries errorCode and optional retryAfterMs", async () => {
		const { WebhookDeliveryError } = await import(
			"../handlers/webhook-delivery"
		);

		const err = new WebhookDeliveryError(
			"rate limited",
			"JOB_RATE_LIMITED",
			30_000,
		);
...
describe("type exports", () => {
	...
	it("BulkJobResult has required shape", () => {
		const result = { jobId: 1, ok: true } as {
			jobId: number;
			ok: boolean;
			error?: string;
		};
