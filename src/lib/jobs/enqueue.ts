...
function measureDepthAndKeys(
	obj: unknown,
	depth = 0,
): { maxDepth: number; keys: number } {
	if (depth > MAX_DEPTH) return { maxDepth: depth, keys: 0 };
	if (obj === null || typeof obj !== "object")
		return { maxDepth: depth, keys: 0 };
	...
	for (const key of entries) {
		const child = measureDepthAndKeys(
...
export async function enqueue(
	...
	if (idempotencyKey) {
		...
		throw new EnqueueError(
			"Idempotency conflict but existing row not found",
			"IDEMPOTENCY_ERROR",
		);
	}
