...
export class ComparisonEngine {
	private providerEndpoints: Record<
		string,
	...
	private async callModel(
		...
		if (!apiKey)
			throw new Error(
				`No API key configured for provider: ${mapping.provider}`,
			);
		...
		try {
			...
			logger.error(`ComparisonEngine: ${modelId} failed`, {
				error: errorMessage,
			});
			return {
				modelId,
	...
	private async getProviderKey(
		...
		const key = await providerKeysService.getActiveProviderKey(
			organizationId,
			provider,
		);
