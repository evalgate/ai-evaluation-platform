...
export function toAssertionsEnvelope(
...
export function safetyPassRateFromEnvelope(
	envelope: AssertionsEnvelope,
): number {
	const safetyAssertions = envelope.assertions.filter(
		(a) => a.category === "safety",
	);
...
export const KNOWN_ASSERTION_KEYS = [
	"pii",
	"toxicity",
	"json_schema",
	"functional",
...
export function validateAssertionsEnvelope(
	...
	const assertions = envelope.assertions.filter((a) =>
		known.has(a.key as (typeof KNOWN_ASSERTION_KEYS)[number]),
	);
	return { version: "v1", assertions };
}
 ...
 */
export function normalizeAssertionsForWrite(
	val: unknown,
): AssertionsEnvelope | null {
	if (val == null) return null;
	...
	return validateAssertionsEnvelope(
		toAssertionsEnvelope(parsed as Record<string, boolean>),
	);
}

/** Compute safety pass rate from parsed value (envelope or legacy) */
export function computeSafetyPassRate(
	parsed: AssertionsEnvelope | LegacyAssertions | null,
): number | null {
	if (!parsed) return null;
	if ("version" in parsed && parsed.version === "v1") {
	...
	const hasunknown =
		legacy.pii !== undefined ||
		legacy.toxicity !== undefined ||
		legacy.harmful !== undefined;
