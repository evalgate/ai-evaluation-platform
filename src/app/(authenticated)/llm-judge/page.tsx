...
export default function LLMJudgePage() {
	...
	const avgScore =
		judgeResults.length > 0
			? judgeResults.reduce((sum: number, r: JudgeResult) => sum + r.score, 0) /
				judgeResults.length
			: 0;

	const highScores = judgeResults.filter(
		(r: JudgeResult) => r.score >= 0.8,
	).length;
	const lowScores = judgeResults.filter(
		(r: JudgeResult) => r.score < 0.5,
	).length;
			...
			{isDemo && (
				<div className="p-4 bg-blue-500/10 border border-blue-500/20 rounded-lg">
					<p className="text-sm text-blue-600 dark:text-blue-400">
						<strong>👁️ Demo Mode:</strong> You're viewing a read-only demo
						project.{" "}
						<Link href="/auth/sign-up" className="underline font-semibold">
							Sign up
						</Link>{" "}
						to create your own LLM judge configurations and duplicate this
						workspace.
					</p>
				</div>
			)}
				...
				<Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
					...
					<DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
						<DialogHeader>
							...
							<DialogDescription>
								Set up a reusable LLM judge with custom evaluation criteria and
								advanced settings
							</DialogDescription>
						</DialogHeader>
							...
							<Tabs defaultValue="basic" className="w-full">
								...
								<TabsContent value="basic" className="space-y-4 mt-4">
									{selectedTemplate !== "custom" && (
										<div className="rounded-lg bg-primary/10 p-3 text-sm">
											<div className="flex items-center gap-2 text-primary font-medium mb-1">
												...
												Using{" "}
												{
													JUDGE_TEMPLATES.find((t) => t.id === selectedTemplate)
														?.name
												}{" "}
												template
											</div>
									...
									<div className="space-y-2">
										...
										<p className="text-xs text-muted-foreground">
											Use placeholders: {"{input}"}, {"{output}"},{" "}
											{"{expected}"}, {"{context}"}
										</p>
									</div>

									<div className="space-y-2">
										<Label htmlFor="config-rubric">
								...
								<TabsContent value="advanced" className="space-y-6 mt-4">
									...
									<div className="space-y-3">
										...
										<p className="text-xs text-muted-foreground">
											Use multiple models and aggregate their scores for more
											reliable evaluation
										</p>
										...
										{enableMultiJudge && (
											<div className="space-y-3 pl-6 border-l-2 border-primary/20">
												<div className="space-y-2">
													...
													<div className="space-y-2">
														{["claude-3-5-sonnet", "gemini-1.5-pro"].map(
															(m) => (
																<div
																	key={m}
												...
												<div className="space-y-2">
													...
													<Select
														...
														onValueChange={setConsensusMethod}
													>
									...
									<div className="space-y-3">
										...
										<div className="space-y-2">
											<Select
												value={costQualityBalance}
												onValueChange={setCostQualityBalance}
											>
												<SelectTrigger>
													<SelectValue />
												</SelectTrigger>
								...
								<CardHeader className="pb-3">
									...
									{config.template && (
										<CardDescription className="text-xs">
											Based on{" "}
											{
												JUDGE_TEMPLATES.find((t) => t.id === config.template)
													?.name
											}{" "}
											template
										</CardDescription>
