...
export default function AnnotationsPage() {
	...
	// Advanced settings
	const [requireMultipleAnnotators, setRequireMultipleAnnotators] =
		useState(false);
	...
	const handleCreateTask = async () => {
		...
		const { data: checkResult } = await check({
			featureId: "annotations",
			requiredBalance: 1,
		});
		if (!checkResult?.allowed) {
			toast.error(
				"You've reached your annotation task limit. Please upgrade your plan.",
			);
		...
		try {
			...
			if (!response.ok) {
				...
				if (response.status === 402) {
					toast.error(
						"Annotation task limit reached. Please upgrade your plan.",
					);
				} else {
			...
			toast.success(
				template ? "Task created from template!" : "Annotation task created!",
			);
	...
	const percentage = hasLimit
		? Math.min(100, (annotationsUsed / annotationsLimit) * 100)
		: 0;
					...
					<Dialog open={showCreateDialog} onOpenChange={setShowCreateDialog}>
						...
						<DialogContent className="max-w-3xl max-h-[90vh] overflow-y-auto">
							<DialogHeader>
								...
								<DialogDescription>
									Set up a task for human annotators to review and rate model
									outputs with advanced quality controls
								</DialogDescription>
							</DialogHeader>
								...
								<Tabs defaultValue="basic" className="w-full">
									...
									<TabsContent value="basic" className="space-y-4 mt-4">
										...
										<div className="space-y-2">
											...
											<p className="text-xs text-muted-foreground">
												Clear instructions help annotators provide consistent,
												quality feedback
											</p>
										</div>
									</TabsContent>
									...
									<TabsContent value="annotators" className="space-y-6 mt-4">
										...
										<div className="space-y-3">
											...
											<p className="text-xs text-muted-foreground">
												Have multiple annotators review each item for more
												reliable results
											</p>
										...
										<div className="space-y-3">
											...
											<div className="space-y-2">
												<Select
													value={annotatorAssignment}
													onValueChange={setAnnotatorAssignment}
												>
													<SelectTrigger>
														<SelectValue />
													</SelectTrigger>
										...
										<div className="space-y-3">
											...
											<p className="text-xs text-muted-foreground">
												Enable review and approval workflow for annotations
												before finalizing
											</p>
										</div>
									</TabsContent>
									...
									<TabsContent value="quality" className="space-y-6 mt-4">
										...
										<div className="space-y-3">
											...
											<p className="text-xs text-muted-foreground">
												Use gold standard items and accuracy tracking to ensure
												annotation quality
											</p>
												...
												<div className="space-y-3 pl-6 border-l-2 border-primary/20">
													<div className="space-y-2">
														...
														<Slider
															...
															onValueChange={([value]) =>
																setGoldStandardPercentage(value)
															}
														/>
														<p className="text-xs text-muted-foreground">
															Percentage of items with known correct answers for
															quality checks
														</p>
													</div>
													...
													<div className="space-y-2">
														<Label htmlFor="accuracy-threshold">
															Minimum Accuracy:{" "}
															{(minAccuracyThreshold * 100).toFixed(0)}%
														</Label>
														<Slider
															...
															onValueChange={([value]) =>
																setMinAccuracyThreshold(value)
															}
														/>
														<p className="text-xs text-muted-foreground">
															Required accuracy on gold standard items to
															continue annotating
														</p>
													</div>
													...
													<div className="rounded-lg bg-yellow-500/10 p-3 text-sm">
														<p className="text-yellow-700 dark:text-yellow-400 text-xs">
															<strong>Note:</strong> Annotators below the
															accuracy threshold will be notified and may need
															retraining
														</p>
													</div>
												</div>
										...
										<div className="space-y-3">
											...
											<div className="space-y-2 text-xs text-muted-foreground">
												<div className="flex items-center gap-2">
													...
													<span>
														Inter-annotator agreement (when multiple annotators
														enabled)
													</span>
												</div>
												<div className="flex items-center gap-2">
													<CheckSquare className="h-3 w-3" />
													<span>
														Time per annotation (to identify anomalies)
													</span>
												</div>
												<div className="flex items-center gap-2">
													<CheckSquare className="h-3 w-3" />
					...
					<CardContent className="p-4 flex items-start gap-3">
						...
						<div className="flex-1">
							...
							<p className="text-sm text-yellow-800 dark:text-yellow-200 mt-1">
								You've used {annotationsUsed} of {annotationsLimit} annotation
								tasks this month. Upgrade your plan for more tasks.
							</p>
					...
					<Card className="border-dashed">
						<CardContent className="flex flex-col items-center justify-center py-12 sm:py-16 px-4">
							...
							<p className="text-xs sm:text-sm text-muted-foreground text-center mb-4 sm:mb-6 max-w-sm">
								Create your first annotation task to start collecting human
								feedback with quality controls
							</p>
							<Button
