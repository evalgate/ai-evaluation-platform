			...
			<main className="mx-auto max-w-4xl px-4 sm:px-6 py-8 sm:py-12 flex-1">
				...
				<div className="mb-6 sm:mb-8">
					...
					<p className="text-base sm:text-lg text-muted-foreground">
						Add evaluation and tracing to your LangChain applications with
				...
				<div className="prose prose-sm sm:prose-base max-w-none">
					...
					<p>
						LangChain makes it easy to build complex LLM applications, but that
						complexity creates more failure points. Proper evaluation ensures
						your chains, agents, and RAG systems work reliably in production.
					</p>
...
# Navigate to /traces and filter by "customer-support-chain"`}
					...
					<ul>
						...
						<li>
							<strong>Token budget:</strong> Alert if daily tokens exceed
							threshold
						</li>
					</ul>
...
result = ai_eval.trace(name="custom-chain")(custom_chain)({"input": "..."})`}
					...
					<ul>
						<li>
							<strong>High level:</strong> Trace entire chain for end-to-end
							monitoring
						</li>
						<li>
							<strong>Mid level:</strong> Add spans for key steps (retrieval,
							tool calls)
						</li>
						<li>
							<strong>Low level:</strong> Trace individual LLM calls for
							debugging
						</li>
					</ul>
...
chain.run("...", callbacks=[EvalCallbackHandler()])`}
					...
					<p>
						Ensure SDK is initialized with correct API key and the trace
					...
					<p>
						Use context managers (`with ai_eval.span(...)`) for manual span
						creation in custom chains.
					</p>
					...
					<p>
						Sample traces (e.g., trace 10% of requests) for high-throughput
						applications.
					</p>
					...
					<div className="bg-card border border-border p-6 rounded-lg my-6">
						...
						<p>
							<strong>Setup:</strong> LangChain agent with 5 tools (search,
							database, calculator, email, escalation)
						</p>
						<p>
