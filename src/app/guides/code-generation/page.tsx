		...
		<div className="min-h-screen bg-background text-foreground flex flex-col">
			...
			<main className="mx-auto max-w-4xl px-4 sm:px-6 py-8 sm:py-12 flex-1">
				...
				<div className="prose prose-sm sm:prose-base max-w-none">
					...
					<p>
						LLMs can write code quickly, but that code can contain bugs,
						security vulnerabilities, or logic errors that break production
						systems. Unlike text generation, code must be functionally correct,
...
assert(commentRatio >= 0.1); // 10% comments`}
					...
					<div className="bg-muted p-4 rounded-lg text-sm my-4">
						...
						<p className="mb-0">
							"Review this code for logical correctness, efficiency, and edge
							case handling. Score 1-5 on: (1) Correctness: Does the logic match
							the requirements? (2) Efficiency: Is the time complexity
							reasonable? (3) Robustness: Does it handle edge cases? Provide
  ...
  "tests": [
    ...
    { "input": null, "shouldThrow": true }
					...
					<div className="bg-muted p-4 rounded-lg text-sm my-4">
						...
						<p className="mb-0">
							<strong>Clear:</strong> "Write a function sort(arr) that sorts an
							array of integers in ascending order. Should handle empty arrays
							and duplicates. Use O(n log n) algorithm."
						</p>
					</div>
...
if (failedTests.length > 0) {
  ...
  const revisedCode = await llm.generate({
    ...
    feedback: feedback
					...
					<ol>
						<li>
							<strong>Shadow mode:</strong> Generate code but don't execute
							(compare to human-written)
						</li>
						<li>
