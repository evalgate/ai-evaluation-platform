...
export const GET = secureRoute(async (req: NextRequest, ctx: AuthContext) => {
	...
	const query = db
		...
		.innerJoin(
			evaluationRuns,
			eq(humanAnnotations.evaluationRunId, evaluationRuns.id),
		)
...
export const POST = secureRoute(async (req: NextRequest, ctx: AuthContext) => {
	...
	const { evaluationRunId, testCaseId, rating, feedback, labels, metadata } =
		body;

	if (!evaluationRunId || !testCaseId) {
		return validationError(
			"Missing required fields: evaluationRunId and testCaseId",
		);
	}
