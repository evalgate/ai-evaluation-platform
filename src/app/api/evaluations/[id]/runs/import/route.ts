...
import { db } from "@/db";
import { evaluationRuns, evaluations, qualityScores, testCases, testResults } from "@/db/schema";
...
export const POST = secureRoute(
  async (req: NextRequest, ctx: AuthContext, params) => {
    ...
    if (invalid.length > 0) {
      return validationError(`Test case IDs not found in evaluation: ${invalid.join(", ")}`);
    }
    ...
    if (idempotencyKey?.trim()) {
      ...
      if (existing) {
        ...
        const flags =
          (typeof qs?.flags === "string" ? JSON.parse(qs.flags || "[]") : qs?.flags) ?? [];
        const baseUrl =
