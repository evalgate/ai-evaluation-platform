...
export const GET = secureRoute(
  async (_req: NextRequest, ctx: AuthContext, params) => {
    ...
    if (!run || run.organizationId !== ctx.organizationId) return notFound("Run not found");
    ...
    if (evaluation.type === "human_eval") {
      ...
      const execSettings = evaluation.executionSettings as {
        humanEvalCriteria?: Array<{ name: string; description?: string; scale?: string }>;
      } | null;
      ...
      additionalData = {
        ...
        aggregateMetrics:
          judgeResults.length > 0
            ? {
                average_score:
                  judgeResults.reduce((s, j) => s + (j.score ?? 0), 0) / judgeResults.length,
