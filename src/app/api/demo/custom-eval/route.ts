		...
		switch (id) {
			...
			case "not-hallucinated": {
				if (!opts.expectedOutput) {
					return {
						...
						passed: false,
						expected: "Ground truth provided",
						actual: "No expected output",
						message: "Expected output is required for hallucination check",
					};
				}
				const facts = opts.expectedOutput
					.split(/[.!?]+/)
					.map((s) => s.trim())
					.filter(Boolean);
			...
			case "contains-keywords": {
				if (!opts.keywords || opts.keywords.length === 0) {
					return {
						...
						passed: false,
						expected: "Keywords provided",
						actual: "No keywords",
						message: "Keywords are required for this assertion",
					};
				}
			...
			case "matches-expected": {
				if (!opts.expectedOutput) {
					return {
						...
						passed: false,
						expected: "Expected output provided",
						actual: "No expected output",
						message: "Expected output is required for exact match",
					};
				}
			...
			default:
				return {
					...
					actual: `Unknown assertion: ${id}`,
					message: `Unknown assertion ID: ${id}`,
				};
		}
	} catch (error) {
		return {
			...
			try {
				...
				if (
					...
					return validationError(
						"assertions must be a non-empty array of assertion IDs",
					);
				}
				...
				return NextResponse.json({
					...
					results: {
						...
						tests: results.map((r, i) => ({
							...
							notes:
								r.message ||
								(r.passed ? `${r.name} passed` : `${r.name} failed`),
						})),
					},
					qualityScore,
				});
