  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 16, height: "100%" }}>
      <div style={{ ...card, flex: 1, padding: 24 }}>
        <div style={{ fontSize: 20, fontWeight: 600, marginBottom: 4 }}>Workflow Graph</div>
        ...
        >
          ...
          {WORKFLOW_EDGES.map((edge, i) => {
            ...
            const tx = to.x,
              ty = to.y + NODE_H / 2;
            const mx = (fx + tx) / 2;
            const edgeOpacity = interpolate(frame, [i * 5, i * 5 + 15], [0, 0.5], {
          ...
          {WORKFLOW_NODES.map((node, i) => {
            ...
            const nodeOpacity = interpolate(frame, [i * 6, i * 6 + 12], [0, 1], {
              extrapolateRight: "clamp",
              extrapolateLeft: "clamp",
            });
            return (
              <g key={node.id} transform={`translate(${node.x}, ${node.y})`} opacity={nodeOpacity}>
                <rect
                  width={NODE_W}
                  height={NODE_H}
                  rx={10}
                ...
                <text x={18} y={28} fontSize={15} fill={colors.text} fontWeight={600}>
