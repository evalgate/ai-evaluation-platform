import { db } from "@/db";
import { apiKeys, apiUsageLogs, user } from "@/db/schema";
...
async function main() {
	...
	if (validApiKeys.length === 0 || validUsers.length === 0) {
		throw new Error(
			"Please seed api_keys and user tables first before seeding api_usage_logs",
		);
	}
	...
	for (let i = 0; i < 100; i++) {
		...
		const responseTimeRange =
			responseTimeRanges[
				selectedMethod.method as keyof typeof responseTimeRanges
			];
		const responseTime = getRandomInt(
	...
	console.log(
		`   - ${sampleLogs.filter((l) => l.apiKeyId).length} with API key authentication`,
	);
	console.log(
		`   - ${sampleLogs.filter((l) => l.userId).length} with user authentication`,
	);
}
