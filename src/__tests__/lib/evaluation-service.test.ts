...
describe("EvaluationService business logic", () => {
  ...
  it("list paginates and scopes evaluations by organization + status", async () => {
    ...
    const results = await evaluationService.list(42, { limit: 10, offset: 5, status: "active" });
  ...
  it("create stores evaluation metadata and associated test cases", async () => {
    const payload = {
      ...
      testCases: [{ input: "input 2", expectedOutput: "expected", metadata: { foo: "bar" } }],
    };
    ...
    expect((testCaseInsertValues![0] as Record<string, unknown>).input).toBe("input 2");
  });
});
