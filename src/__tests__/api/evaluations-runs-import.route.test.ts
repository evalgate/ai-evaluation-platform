  ...
  const builder: Record<string, unknown> = {
    ...
    then: (onFulfilled: (v: unknown) => unknown) => {
      ...
      if (t === "evaluations") {
        ...
        return Promise.resolve([{ id: 42, organizationId: 1 }]).then(onFulfilled);
      }
...
describe("POST /api/evaluations/[id]/runs/import", () => {
  ...
  it("returns 400 on invalid payload", async () => {
    ...
    const req = new NextRequest("http://localhost/api/evaluations/42/runs/import", {
      method: "POST",
  ...
  it("supports idempotent import (same run twice) by returning success both times", async () => {
    ...
    const req1 = new NextRequest("http://localhost/api/evaluations/42/runs/import", {
      method: "POST",
      body: JSON.stringify({}),
    });
    ...
    if (res1.status >= 400) {
      ...
      throw new Error(`First POST failed: status=${res1.status} body=${JSON.stringify(body)}`);
    }

    const req2 = new NextRequest("http://localhost/api/evaluations/42/runs/import", {
      method: "POST",
      body: JSON.stringify({}),
    });
    ...
    if (res2.status >= 400) {
      ...
      throw new Error(`Second POST failed: status=${res2.status} body=${JSON.stringify(body)}`);
    }
