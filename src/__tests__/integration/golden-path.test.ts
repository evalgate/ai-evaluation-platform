...
import { normalizeRole, hasMinRole, hasScopes } from '@/lib/api/secure-route';
import { scopesForRole, SCOPES } from '@/lib/auth/scopes';
import { wilsonConfidence } from '@/lib/services/aggregate-metrics.service';
import { EXIT, type CheckArgs } from '@/packages/sdk/src/cli/check';
...
describe('Golden-path lifecycle (unit-level)', () => {
  ...
  describe('Auth & scopes integration', () => {
    it('owner has all scopes', () => {
      ...
      expect(hasScopes(scopes, [SCOPES.ADMIN_ORG, SCOPES.EVAL_WRITE, SCOPES.REPORTS_WRITE])).toBe(true);
    });
