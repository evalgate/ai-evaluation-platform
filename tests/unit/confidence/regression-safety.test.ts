...
import { type UnitTestExportData, validateExportData } from "@/lib/export-templates";
import { assertNoSecrets, prepareExportForShare, sanitizeExportData } from "@/lib/shared-exports";
...
describe("Score regression tolerance", () => {
  ...
  it("current score is within tolerance of baseline", () => {
    ...
    expect(current.overall).toBeGreaterThanOrEqual(baselineScore.overall - TOLERANCE);
  });
...
describe("Export secret detection", () => {
  ...
  it("allows clean export data through", () => {
    const clean = {
      evaluation: { id: "1", name: "Test", description: "A clean eval", type: "unit_test" },
      timestamp: "2025-01-01T00:00:00Z",
      summary: { totalTests: 1, passed: 1, failed: 0, passRate: "100%" },
    };
