...
describe("validatePayload", () => {
  ...
  it("should reject negative webhookId", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error).toContain("webhookId: Number must be greater than 0");
    }
  });
  ...
  it("should reject negative organizationId", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error).toContain("organizationId: Number must be greater than 0");
    }
  });
  ...
  it("should reject empty event string", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error).toContain("event: String must contain at least 1 character(s)");
    }
  });
  ...
  it("should reject empty timestamp string", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error).toContain("timestamp: String must contain at least 1 character(s)");
    }
  });
  ...
  it("should reject non-integer webhookId", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error).toContain("webhookId: Expected integer, received float");
    }
  });
  ...
  it("should reject non-integer organizationId", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error).toContain("organizationId: Expected integer, received float");
    }
  });
  ...
  it("should reject string webhookId", () => {
    ...
    expect(result.success).toBe(false);
    if (!result.success) {
      expect(result.error).toContain("webhookId: Expected number, received string");
    }
  });
