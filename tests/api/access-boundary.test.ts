...
describe("Access boundary — cross-tenant isolation", () => {
  ...
  async function assertNormalizedEnvelope(res: Response, expectedStatus: number) {
  ...
  it("Tenant B cannot list Tenant A's runs (404)", async () => {
    ...
    const req = new NextRequest(`http://localhost/api/evaluations/${EVAL_A_ID}/runs`);
  ...
  it("Tenant B cannot get Tenant A's run by ID (404)", async () => {
    ...
    const req = new NextRequest(`http://localhost/api/evaluations/${EVAL_A_ID}/runs/${RUN_A_ID}`);
    const res = await getRun(req, {
      params: Promise.resolve({ id: String(EVAL_A_ID), runId: String(RUN_A_ID) }),
    });
  ...
  it("Tenant B with guessed eval ID gets 404 (no info leak)", async () => {
    ...
    const req = new NextRequest(`http://localhost/api/evaluations/${guessedEvalId}/runs`);
    const res = await getRuns(req, {
