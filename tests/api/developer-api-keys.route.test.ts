	...
	secureRoute: (handler: unknown, opts?: { minRole?: string }) => {
		return async (
			...
			if (opts?.minRole === "admin") {
				const allowed =
					state.ctx.role === "admin" || state.ctx.role === "owner";
				if (!allowed) {
					return new Response(
...
describe("/api/developer/api-keys", () => {
	...
	it("POST blocks key prefix collisions", async () => {
		...
		const uniqueError = new Error(
			"UNIQUE constraint failed: apiKeys.keyPrefix",
		);
		(uniqueError as Error & { code?: string }).code =
	...
	it("GET requires auth and lists keys", async () => {
		...
		const res = await (
			GET as (req: unknown, opts: unknown) => Promise<Response>
		)(req as unknown, {} as unknown);
