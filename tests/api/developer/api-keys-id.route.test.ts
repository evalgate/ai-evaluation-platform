...
describe("DELETE /api/developer/api-keys/[id]", () => {
	...
	it("sets revokedAt and returns 200", async () => {
		...
		selectQueue.push([
			{ id: 7, organizationId: 1, userId: "u1", revokedAt: null },
		]);

		// ✅ update returns a row after revocation
		h.updateReturning.mockResolvedValueOnce([
			{ id: 7, revokedAt: new Date().toISOString() },
		]);

		const req = new NextRequest("http://localhost/api/developer/api-keys/7", {
			method: "DELETE",
		});
		...
		if (res.status >= 400) {
			...
			throw new Error(
				`DELETE failed: status=${res.status} body=${JSON.stringify(body)}`,
			);
		}
	...
	it("returns 404 when key is not found", async () => {
		...
		if (res.status >= 400 && res.status !== 404) {
			const body = await res.json().catch(() => null);
			throw new Error(
				`Expected 404, got ${res.status}. body=${JSON.stringify(body)}`,
			);
		}
