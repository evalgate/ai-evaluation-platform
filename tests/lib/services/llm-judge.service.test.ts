...
describe("LLMJudgeService", () => {
	...
	describe("listConfigs", () => {
		it("returns configs with pagination", async () => {
			...
			const results = await llmJudgeService.listConfigs(1, {
				limit: 10,
				offset: 5,
			});
	...
	describe("updateConfig", () => {
		it("returns null when config not found", async () => {
			...
			const result = await llmJudgeService.updateConfig(999, 1, {
				name: "Updated",
			});
			expect(result).toBeNull();
		});
	...
	describe("evaluate", () => {
			...
			const mockResponse = new Response(
				JSON.stringify({
					choices: [
						{
							message: {
								content: '{"score": 85, "reasoning": "Good", "passed": true}',
							},
						},
					],
				}),
		...
		it("uses Anthropic provider when model is Claude", async () => {
			...
			const mockResponse = new Response(
				JSON.stringify({
					content: [
						{ text: '{"score": 90, "reasoning": "Excellent", "passed": true}' },
					],
				}),
				{ status: 200 },
			);
			...
			const mockResponse = new Response(
				JSON.stringify({
					choices: [
						{
							message: {
								content: "I give this an 85/100 because it's pretty good.",
							},
						},
					],
				}),
			...
			const mockResponse = new Response(
				JSON.stringify({
					choices: [
						{
							message: {
								content: '{"score": 90, "reasoning": "Good", "passed": true}',
							},
						},
					],
				}),
								...
								choices: [
									{
										message: {
											content:
												'{"score": 90, "reasoning": "Good", "passed": true}',
										},
									},
								],
							}),
