...
vi.mock("@/lib/api/request-id", () => ({
	...
	runWithRequestIdAsync: async (_id: string, fn: () => Promise<unknown>) =>
		fn(),
...
vi.mock("@/lib/api-rate-limit", () => ({
	withRateLimit: vi.fn(
		async (_req: unknown, handler: (req: unknown) => Promise<NextResponse>) =>
			handler(_req),
	),
}));
...
describe("secureRoute (priority tests)", () => {
	...
	it("bubbles requireAuthWithOrg denial into an apiError response", async () => {
		...
		const response = new Response(
			JSON.stringify({ code: "UNAUTHORIZED", error: "nope" }),
			{
				status: 401,
