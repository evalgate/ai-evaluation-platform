...
function hasSensitiveKeys(obj: unknown, depth = 0): boolean {
	...
	if (typeof obj === "object") {
		for (const [key] of Object.entries(obj)) {
			...
			if (hasSensitiveKeys((obj as Record<string, unknown>)[key], depth + 1))
				return true;
		}
	}
	return false;
...
describe("sanitizeExportData + assertNoSecrets", () => {
	...
	it("rejects Bearer token in value", () => {
		const withBearer = {
			...baseExport,
			testResults: [
				{ output: "Bearer eyJhbGciOiJIUzI1NiIsInR5cCI6IkpXVCJ9.xxx.yyy" },
			],
		};
...
describe("stableStringify + computeExportHash", () => {
	...
	it("computeExportHash is stable for nested key order", () => {
		const a = {
			evaluation: { name: "Test", id: "1" },
			summary: { passed: 8, failed: 2 },
		};
		const b = {
			summary: { failed: 2, passed: 8 },
