...
describe("formatExportData", () => {
	...
	it("formats human_eval export data correctly", () => {
		...
		const additionalData = {
			...
			criteria: [
				{ name: "Quality", description: "Overall quality", scale: "1-5" },
			],
		};
	...
	it("throws error for unsupported type", () => {
		...
		expect(() => formatExportData(baseData, {})).toThrow(
...
describe("getExportDescription", () => {
	...
	it("returns correct description for model_eval", () => {
		expect(getExportDescription("model_eval")).toBe(
			"LLM judge evaluations with aggregate metrics",
		);
	});
