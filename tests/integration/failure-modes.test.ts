...
import { prepareExportForShare, sanitizeExportData } from "@/lib/shared-exports";
...
describe("Nonexistent resource handling", () => {
  ...
  it("update returns null for nonexistent evaluation", async () => {
    if (!dbReady) return;
    const result = await evaluationService.update(999999, ORG_ID, { name: "nope" });
    expect(result).toBeNull();
  });
...
describe("Export sanitization failure modes", () => {
  it("rejects null export data", () => {
    expect(() => sanitizeExportData(null)).toThrow("Export data must be a non-null object");
  });

  it("rejects non-object export data", () => {
    expect(() => sanitizeExportData("string" as unknown as Record<string, unknown>)).toThrow();
  });
  ...
  it("rejects export with secret-like keys", () => {
    const withSecret = {
      evaluation: { id: "1", name: "Test", apiKey: "sk-123456789012345678901234567890" },
    };
