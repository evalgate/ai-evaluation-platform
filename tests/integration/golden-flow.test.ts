...
describe("Golden Flow: create → run → export → validate", () => {
  ...
  it("Step 1: creates an evaluation with test cases", async () => {
    ...
    const cases = await db.select().from(testCases).where(eq(testCases.evaluationId, evalId));
  ...
  it("Step 4: constructs and validates an export payload", async () => {
    ...
    const hash1 = computeExportHash(exportData as unknown as Record<string, unknown>);
    const hash2 = computeExportHash(exportData as unknown as Record<string, unknown>);
