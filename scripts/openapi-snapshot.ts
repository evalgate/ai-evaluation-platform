...
const HASH_FILE = path.resolve(
	process.cwd(),
	"scripts/.openapi-spec-hash.json",
);
const SDK_VERSION_FILE = path.resolve(
	process.cwd(),
	"src/packages/sdk/src/version.ts",
);
...
function main(): number {
	...
	const specMatch = sdkVersionContent.match(
		/SPEC_VERSION\s*=\s*["']([^"']+)["']/,
	);
	...
	console.log(
		`Updated ${HASH_FILE} (version: ${version}, hash: ${currentHash})`,
	);
	return 0;
}
