...
const CONFIDENCE_SUMMARY_PATH = path.resolve(
	process.cwd(),
	"evals/confidence-summary.json",
);
...
function writeReport(
	...
	const report = {
		...
		failures,
		baseline: baseline
			? { updatedAt: baseline.updatedAt, updatedBy: baseline.updatedBy }
			: null,
	};
	try {
...
function printSummary(
	...
	if (category === "infra_error") {
		console.log(
			"║  🔧 INFRA ERROR — gate could not run                        ║",
		);
		console.log(
			"╠══════════════════════════════════════════════════════════════╣",
		);
		...
		console.log(
			"║  Fix regressions or update baseline:                        ║",
		);
		console.log(
...
function runGate(): number {
	...
	console.log("╚══════════════════════════════════════════╝\n");
	console.log(
		`Baseline from: ${baseline.updatedAt} (by ${baseline.updatedBy})`,
	);
	console.log(
	...
	try {
		...
		goldenPassRate =
			goldenTotal > 0 ? Math.round((goldenPassed / goldenTotal) * 100) : 0;
	} catch {
	...
	deltas.push({
		...
		status:
			scoreDelta < -baseline.tolerance.scoreDrop
				? "fail"
				: scoreDelta < 0
					? "warn"
					: "pass",
	...
	if (!summary) {
		console.error(
			"❌ Confidence summary not found: evals/confidence-summary.json",
		);
		console.error(
			"   This file is emitted by pnpm test:confidence. The command may have crashed.",
		);
		writeReport(
	...
	if (baseline.productMetrics?.p95ApiLatencyMs != null) {
		...
		const latencyResultsPath = path.resolve(
			process.cwd(),
			"evals/latency-benchmark.json",
		);
...
function updateBaseline(): number {
	...
	if (existsSync(RESULTS_PATH)) {
		try {
			...
			goldenPassRate =
				goldenTotal > 0 ? Math.round((goldenPassed / goldenTotal) * 100) : 100;
		} catch {
			// Use defaults
		}
	}
	...
	const newBaseline: Baseline = {
		...
		productMetrics: {
			p95ApiLatencyMs:
				p95ApiLatencyMs ?? existing?.productMetrics?.p95ApiLatencyMs,
	...
	console.log(
		`   Unit confidence: ${unitPassed} tests (${unitOk ? "passed" : "FAILED"})`,
	);
	console.log(
		`   DB confidence: ${dbPassed} tests (${dbOk ? "passed" : "FAILED"})`,
	);
	if (unitDur != null) console.log(`   Unit lane: ${unitDur}ms`);
	if (dbDur != null) console.log(`   DB lane: ${dbDur}ms`);
	if (p95ApiLatencyMs != null)
		console.log(`   p95 API latency: ${p95ApiLatencyMs}ms`);
	console.log();
