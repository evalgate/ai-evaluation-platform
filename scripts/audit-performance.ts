...
async function main(): Promise<number> {
  ...
  if (initMs > SDK_INIT_MAX_MS) {
    console.error(`audit:performance — SDK init: ${initMs.toFixed(0)}ms > ${SDK_INIT_MAX_MS}ms`);
    failed = true;
  } else {
  ...
  const suite = createTestSuite("perf-audit", {
    cases: [
      { input: "a", assertions: [(o: string) => ({ passed: o === "a", message: "" })] },
      { input: "b", assertions: [(o: string) => ({ passed: o === "b", message: "" })] },
      { input: "c", assertions: [(o: string) => ({ passed: o === "c", message: "" })] },
    ],
    executor,
  });
