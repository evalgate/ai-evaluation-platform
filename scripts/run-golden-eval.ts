...
async function main(): Promise<number> {
  ...
  if (apiKey) {
    const result = await openAIChatEval({
      ...
      model: "gpt-4o-mini",
      apiKey,
      cases: config.cases.map((c) => ({ input: c.input, expectedOutput: c.expectedOutput })),
    });
    ...
    const outPath = path.resolve(process.cwd(), "evals/golden/golden-results.json");
    ...
    if (!passed) {
      if (score < config.minScore) console.error(`  score ${score} < minScore ${config.minScore}`);
  ...
  const suiteCases = config.cases.map((c) => ({
    ...
    assertions: [
      (output: string) =>
        expect(output).toContainKeywords(c.expectedOutput.split(/\s+/).filter(Boolean)),
    ],
  }));
  ...
  if (!passed) {
    ...
    if (result.passed !== result.total) {
      ...
      for (const r of result.results.filter((x: { passed: boolean }) => !x.passed)) {
        console.error(`  - "${r.input}" → expected: ${r.expected}, got: ${r.actual}`);
      }
    }
    return 1;
  }
